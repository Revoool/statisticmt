...
with tab3:
    ...
    # -------------------------------
    # 💡 Подкатегории с риском недооценки
    # -------------------------------
    ...
    # Безопасный расчёт потенциального прироста
    potentials = (
        df[df["subcategory"].isin(candidates["subcategory"])]
        .groupby("subcategory", as_index=False)["Итого продаж"]
        .sum()
    )
    potentials["Потенциал роста при +10%"] = (potentials["Итого продаж"] * 0.10).round(0)

    candidates = candidates.merge(potentials[["subcategory", "Потенциал роста при +10%"]], on="subcategory", how="left")

    st.dataframe(candidates[[
        "subcategory", "Среднее_изменение_цены_проц",
        "Товаров_в_подкатегории", "Потенциал роста при +10%"
    ]].sort_values("Потенциал роста при +10%", ascending=False))

    st.markdown("""
    📌 **Рекомендация:** Подумайте о повышении цен хотя бы на 10% в этих подкатегориях.  
    Даже при умеренном росте цен можно достичь сопоставимого эффекта, как от повышения минимального заказа.
    """)

    # График: средний рост по подкатегориям
    fig3 = px.bar(
    ...
    fig5 = px.bar(
        ...
        x="subcategory",
        y="Среднее_изменение_цены_проц",
        labels={"Среднее_изменение_цены_проц": "Среднее падение, %"},
        color="Среднее_изменение_цены_проц",
    )
