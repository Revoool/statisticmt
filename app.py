...
with tab3:
    ...
    # Расчёт текущей и прогнозируемой выручки
    potentials = (
        df[df["subcategory"].isin(candidates["subcategory"])]
        .groupby("subcategory", as_index=False)["Итого продаж"]
        .sum()
    ...
    # Добавим расчёт после +10%
    potentials["Итого продаж (если +10%)"] = (potentials["Итого продаж (до)"] * 1.10).round(0)
    potentials["Потенциал роста при +10%"] = (potentials["Итого продаж (если +10%)"] - potentials["Итого продаж (до)"]).round(0)

    # Объединяем с таблицей кандидатов
